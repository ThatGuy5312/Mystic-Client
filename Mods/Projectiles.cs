...
namespace MysticClient.Mods
{
    public class Projectiles : ProjectileLib
    {
        ...
        public static string[] sizeNames = { "Normal", ".5x", "2x", "3x", "4x", "5x" };
        public static float[] sizes = { 1, .5f, 2, 3, 4, 5 };
        ...
        public static void ProjectileHalo()
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                var data = new ProjectileData(projectile, trail, pos, Vector3.zero, rainbowColor ? RGBColor() : funnyRGB ? HardColor(Random.Range(0, 10)) : projectileColor, projSize);
                if (GetEnabled("ServerSided Projectiles"))
                    ServerSided.FireProjectile(data);
                else
                    LaunchProjectile(data);
