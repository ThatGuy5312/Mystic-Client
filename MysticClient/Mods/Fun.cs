...
namespace MysticClient.Mods
{
    public class Fun : GunLib
    {
        ...
        public static void GliderTrails()
        {
            foreach (var gliders in Gliders())
            {
                var trail = gliders.AddComponent<TrailRenderer>();
                trail.material = new Material(Shader.Find("Sprites/Default"));
                trail.time = 1f;
                trail.startWidth = 1f;
                trail.endWidth = 0f;
                trail.minVertexDistance = 6f;
                trail.material.color = Projectiles.funnyRGB ? HardColor(Random.Range(0, 10)) : RGBColor();
            }
        }
        public static void GliderTrailsOff()
        {
            foreach (var gliders in Gliders())
                Destroy(gliders.GetComponent<TrailRenderer>());
        }
        ...
        public static void GetGliderOwnership(GliderHoldable glider)
        {
            glider.OnHover(null, null);
            glider.OnGrab(null, null);
            glider.GetView.OwnershipTransfer = OwnershipOption.Request;
            glider.RequestOwnership();
            glider.GetView.ViewID = RigUtils.GetViewFromPlayer(PhotonSystem.LocalPlayer).ViewID;
            glider.GetView.ControllerActorNr = PhotonSystem.LocalPlayer.ActorNumber;
            glider.GetView.OwnerActorNr = PhotonSystem.LocalPlayer.ActorNumber;
        }
        ...
        public static void WaterRight(string btnname)
        {
            if (PhotonSystem.InRoom)
            {
                ...
                {
                    RPCManager.WaterEvent(RpcTarget.All, new object[]
                    {
                        RigUtils.MyOnlineRig.rightHandTransform.position,
                        RigUtils.MyOnlineRig.rightHandTransform.rotation,
                    });
                    RPCProtection();
                }
            }
        ...
        public static void WaterLeft(string btnname)
        {
            if (PhotonSystem.InRoom)
            {
                if (Controller.leftGrab || UserInput.GetMouseButton(0))
                {
                    RPCManager.WaterEvent(RpcTarget.All, new object[]
                    {
                        RigUtils.MyOnlineRig.leftHandTransform.position,
                        RigUtils.MyOnlineRig.leftHandTransform.rotation,
                    });
                    RPCProtection();
                }
            }
        ...
        public static void WaterGun(string btnname)
        {
            if (PhotonSystem.InRoom)
            {
                if (CreateGun())
                {
                    ...
                    RigUtils.MyOfflineRig.transform.position = pointer.transform.position - new Vector3(0, 3, 0);
                    RigUtils.MyNetworkView.transform.position = pointer.transform.position - new Vector3(0, 3, 0);
                    RPCManager.WaterEvent(RpcTarget.All, new object[]
                    {
        ...
        public static void PlaySound(int index, string btnname)
        {
            if (PhotonSystem.InRoom)
            {
                if (Controller.rightControllerIndexFloat > 0.3f || UserInput.GetMouseButton(0))
                {
                    RPCManager.SoundEvent(RpcTarget.All, new object[]
                    {
                        index,
                        true,
                        999999f
