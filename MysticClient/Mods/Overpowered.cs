    ...
    public class Overpowered : GunLib
    {
            ...
            if (CreateGun(out VRRig rig))
                foreach (var gliders in Gliders())
                    if (gliders.GetView.Owner == RigUtils.MyRealtimePlayer)
                    {
                        ...
                        Fun.GetGliderOwnership(gliders);
        }
        ...
        public static void TagAura(string tooltip)
        {
            if (PhotonSystem.InRoom)
            {
                if (!IsTagged(PhotonSystem.LocalPlayer))
                {
                    ...
                    {
                        bool toggled = GetEnabled("Toggle Tag Aura (RT)") && Controller.rightControllerIndexFloat > 0.3f ^ UserInput.GetMouseButton(0);
                        if (!IsTagged(RigUtils.GetNetFromRig(rigs)) && !RigUtils.MyPlayer.disableMovement && toggled || !toggled)
                        {
                            if (PhotonSystem.IsMasterClient)
                            {
        ...
        public static void SpamTagSelf(string tooltip)
        {
            if (PhotonSystem.InRoom)
            {
                if (PhotonSystem.IsMasterClient)
                {
                    bool toggled = GetEnabled("Toggle Spam Tag Self (RT)") && Controller.rightControllerIndexFloat > 0.3f ^ UserInput.GetMouseButton(0);
                    if (toggled || !toggled)
        ...
        public static void SpamTagAll(string tooltip)
        {
            if (PhotonSystem.InRoom)
            {
                if (PhotonSystem.IsMasterClient)
                {
                    ...
                    {
                        bool toggled = GetEnabled("Toggle Spam Tag All (RT)") && Controller.rightControllerIndexFloat > 0.3f ^ UserInput.GetMouseButton(0);
                        if (toggled || !toggled)
        ...
        public static void UnTagAll(string tooltip)
        {
            if (PhotonSystem.InRoom)
            {
                if (PhotonSystem.IsMasterClient)
                {
                    ...
                    {
                        bool toggled = GetEnabled("Toggle UnTag All (A)") && Controller.rightControllerIndexFloat > 0.3f ^ UserInput.GetMouseButton(0);
                        if (IsTagged(players) && toggled || !toggled)
                        {
        ...
        public static void TagAll(string tooltip)
        {
            if (PhotonSystem.InRoom)
            {
                bool toggled = GetEnabled("Toggle Tag All (A)") && Controller.rightControllerIndexFloat > 0.3f ^ UserInput.GetMouseButton(0);
                if (PhotonSystem.IsMasterClient && toggled || !toggled)
                {
                    foreach (var players in PhotonSystem.PlayerListOthers)
                    {
                        if (!IsTagged(players))
        ...
        public static void UnTagSelf(string tooltip)
        {
            if (PhotonSystem.InRoom)
            {
                if (PhotonSystem.IsMasterClient)
                {
                    bool toggled = GetEnabled("Toggle UnTag Self (A)") && Controller.rightControllerIndexFloat > 0.3f ^ UserInput.GetMouseButton(0);
                    if (IsTagged(PhotonSystem.LocalPlayer) && toggled || !toggled)
                    {
