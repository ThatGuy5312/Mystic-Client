...
namespace MysticClient.Mods
{
    public class Projectiles : ProjectileLib
    {
        ...
        public static string[] sizeNames = { "Normal", "2x", "3x", "4x", "5x" };
        public static int[] sizes = { 1, 2, 3, 4, 5 };
        ...
        {
            Mode[3]++;
            if (Mode[3] >= Settings.colors.Length) { Mode[3] = 0; }
            projectileColor = Settings.colors[Mode[3]];
            string[] text = GetToolTip(tooltip).buttonText.Split(':');
            GetToolTip(tooltip).buttonText = text[0] + ": " + Settings.Names[0][Mode[3]];
            GetToolTip(tooltip).enabled = false;
        }
        ...
        public static void Cum()
        {
            if (GetEnabled("ServerSided Projectiles"))
            {
                ...
                {
                    var data = new ProjectileData(-675036877, trail, RigUtils.MyOfflineRig.transform.position - new Vector3(0f, 0.3f, 0f), RigUtils.MyOfflineRig.transform.forward.normalized * 5f, Color.white);
            ...
            else
            {
                if (Controller.rightControllerIndexFloat > 0.5f || UserInput.GetMouseButton(0))
                {
                    var data = new ProjectileData(-820530352, trail, RigUtils.MyOfflineRig.transform.position - new Vector3(0f, 0.3f, 0f), RigUtils.MyOfflineRig.transform.forward.normalized * 5f, Color.white);
                    LaunchProjectile(data, true);
                    LegacySendEvent(111, data, new RaiseEventOptions { Receivers = ReceiverGroup.Others }, true);
                }
            }
        }
        public static void Piss()
        {
            if (GetEnabled("ServerSided Projectiles"))
            {
                if (Controller.rightControllerIndexFloat > 0.5f || UserInput.GetMouseButton(0))
                {
                    var data = new ProjectileData(-675036877, trail, RigUtils.MyOfflineRig.transform.position - new Vector3(0f, 0.3f, 0f), RigUtils.MyOfflineRig.transform.forward.normalized * 5f, Color.yellow);
                    ServerSided.FireProjectile(data, true);
                    LegacySendEvent(111, data, new RaiseEventOptions { Receivers = ReceiverGroup.Others }, true);
                }
            }
            else
            {
                if (Controller.rightControllerIndexFloat > 0.5f || UserInput.GetMouseButton(0))
                {
                    var data = new ProjectileData(-820530352, trail, RigUtils.MyOfflineRig.transform.position - new Vector3(0f, 0.3f, 0f), RigUtils.MyOfflineRig.transform.forward.normalized * 5f, Color.yellow);
                    LaunchProjectile(data, true);
                    LegacySendEvent(111, data, new RaiseEventOptions { Receivers = ReceiverGroup.Others }, true);
                }
            }
        ...
        public static void ProjectileGun()
        {
            if (CreateGun())
            {
                var data = new ProjectileData(projectile, trail, pointer.transform.position, Vector3.zero, projectileColor);
                if (GetEnabled("ServerSided Projectiles"))
        ...
        public static void ProjectileHalo()
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                var data = new ProjectileData(projectile, trail, pos, Vector3.zero, funnyRGB ? HardColor(i) : projectileColor);
                if (GetEnabled("ServerSided Projectiles"))
                    ServerSided.FireProjectile(data);
                else
                    LaunchProjectile(data, funnyRGB);
                LegacySendEvent(111, data, new RaiseEventOptions { Receivers = ReceiverGroup.Others }, true);
            }
            // anti lag stuff
